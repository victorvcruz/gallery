...
| `GALLERY_CACHE_MAX_MB` | No | Maximum combined size (in megabytes) of the on-disk image caches (`thumb/`, `preview/`, `full/`). Defaults to `1024`. When exceeded, least-recently-accessed entries are evicted down to ~90% of the cap. `metadata/` is not counted. |
